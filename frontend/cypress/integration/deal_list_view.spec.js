...
describe("Deal list view sort by", function () {
  ...
  it("Can sort deals by rating ascending", function () {
    ...
    cy.intercept("GET", `/api/deals/?ordering=rating`).as("sortBy");
    cy.visit("");
    getToolBarIcon().click();
    cy.wait(2000);
    getToolBarSortBySelector().select("RATING - ASCENDING");
    getToolBarSaveBtn().click();
    cy.get("@sortBy").should((req) => {
      expect(req.request.url).to.equal(
  ...
  it("Can sort deals by created ascending", function () {
    ...
    cy.intercept("GET", `/api/deals/?ordering=created`).as("sortBy");
    cy.visit("");
    getToolBarIcon().click();
    cy.wait(2000);
...
describe("Deal list view filter by category", function () {
  ...
  it("Can filter deals by category", function () {
    ...
    cy.intercept("GET", `/api/deals/?category=GROCERIES`).as("filter");
    cy.visit("");
    getToolBarIcon().click();
    cy.wait(2000);
    getToolBarCategorySelector().select("GROCERIES");
    getToolBarSaveBtn().click();
    cy.get("@filter").should((req) => {
      expect(req.request.url).to.equal(
  ...
  it("Can filter deals by category with & symbol", function () {
    cy.intercept("GET", "/api/deals/", {
      statusCode: 200,
      body: [this.dealOne],
    });
    cy.intercept("GET", `/api/deals/?category=SPORT+%26+LEISURE`).as("filter");
    cy.visit("");
    getToolBarIcon().click();
    cy.wait(2000);
    getToolBarCategorySelector().select("SPORT & LEISURE");
    getToolBarSaveBtn().click();
    cy.get("@filter").should((req) => {
      expect(req.request.url).to.equal(
        "http://127.0.0.1:8000/api/deals/?category=SPORT+%26+LEISURE"
      );
    });
  });
});
...
describe("Deal list view filter by category and sort", function () {
  ...
  it("Can filter deals and sort deals", function () {
    ...
    cy.intercept("GET", `/api/deals/?category=GROCERIES&ordering=created`).as(
      "filterAndCategory"
    );
    ...
    getToolBarSortBySelector().select("UPLOADED - ASCENDING");
    getToolBarSaveBtn().click();
    cy.get("@filterAndCategory").should((req) => {
      expect(req.request.url).to.equal(
  ...
  it("Can filter deals and sort deals with & symbol", function () {
    cy.intercept("GET", "/api/deals/", {
      statusCode: 200,
      body: [this.dealOne],
    });
    cy.intercept(
      "GET",
      `/api/deals/?category=SPORT+%26+LEISURE&ordering=created`
    ).as("filterAndCategory");
    ...
    cy.get("@filterAndCategory").should((req) => {
      expect(req.request.url).to.equal(
        "http://127.0.0.1:8000/api/deals/?category=SPORT+%26+LEISURE&ordering=created"
      );
    });
  });
});
